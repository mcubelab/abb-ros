...
int main(int argc, char** argv)
{
  ...
  Vec transC = Vec("790 0 980",3); // Safe location in the center of the robot workspace. mcube
  Quaternion quat;
  Quaternion quatC = Quaternion("0.0 0.0 1.0 0.0"); // Hand facing down
  ...
  float scale = 150.0;
  // Move in a square
  robot.SetCartesian(transC[0] + scale, transC[1], transC[2], quatC[0], quatC[1], quatC[2], quatC[3]);
  robot.SetCartesian(transC[0] + scale, transC[1] + scale, transC[2], quatC[0], quatC[1], quatC[2], quatC[3]);
  robot.SetCartesian(transC[0], transC[1] + scale, transC[2], quatC[0], quatC[1], quatC[2], quatC[3]);
  robot.SetCartesian(transC[0], transC[1], transC[2], quatC[0], quatC[1], quatC[2], quatC[3]);
