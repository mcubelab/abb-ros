...
#define HAND_CLOSE_ANGLE_finger 80.0  // Finger tip grasp close angle
#define HAND_OPEN_ANGLE_drop 40.0//50.0 // opening angle sufficient enough to drop the object
#define HAND_CLOSE_ANGLE_envelop 85.0  // Enveloping grasp close angle..
#define HAND_OPEN_ANGLE_vibrate 50.0 // Angle dusing vibration
...
// Following things need to be changed if necessary...Nikhil
//static const HomogTransf initialGuess(Quaternion("-0.0009 0.9916 -0.1294 -0.0055").getRotMat(), Vec("0.4812291  0.3341413  0.460",3));
static const HomogTransf initialGuess(Quaternion("-0.0009 0.9916 -0.1294 -0.0055").getRotMat(), Vec("0.4812291  0.3341413  0.4637221",3));
//static const HomogTransf successGuess(Quaternion("-0.0009 0.9916 -0.1294 -0.0055").getRotMat(), Vec("0.4812291  0.3341413  0.460",3));
static const HomogTransf successGuess(Quaternion("-0.0009 0.9916 -0.1294 -0.0055").getRotMat(), Vec("0.4812291  0.3341413  0.4637221",3));
...
{
  ros::init(argc, argv, "regrasp_vibrate");
  ros::NodeHandle node;
  ...
  */
  ...
  ROS_INFO("Regrasp Vibrate started...");
  ...
  strftime (buffer,1024,"vibrate_%Y_%m_%d_%H_%M_%S.txt",timeinfo);
  ...
  {
    regrasp_num+=1;
    ROS_INFO("Regrasp Vibrate %d",regrasp_num);
    ...
    transPick[2]=transPick[2]+ErzPick;
      ...
      hand.SetAngle(HAND_CLOSE_ANGLE_finger);
      hand.WaitRest(0.250);
      ...
      hand.WaitRest(0.250);
      ROS_INFO("Object on the palm");
      
      hand.SetAngle(HAND_CLOSE_ANGLE_envelop);
      hand.WaitRest(0.250);
      ROS_INFO("Hand Closed");
      robot.SetJoints(0,0,0,0,90,0);
      ...
      int sleeptime= 2000000; //15710*1000/minfingerAngle;   // 91750 to 91800, perfornamnce is not constant
      printf("sleep time = %d \n", sleeptime);
      ...
      */
      ...
      hand.SetAngle(HAND_CLOSE_ANGLE_envelop);
      hand.WaitRest(0.250);
      ROS_INFO("Hand Closed after regrasp");
...
double time_in_mill = (tv.tv_sec) * 1000.0 + (tv.tv_usec) / 1000.0 ;
    ...
    printf("transPlace0=%lf\t transPlace1=%lf\t transPlace2=%lf\t quatPlace0=%lf\t quatPlace1=%lf\t quatPlace2=%lf\t quatPlace3=%lf\n",transPlace[0], transPlace[1], transPlace[2], quatPlace[0], quatPlace[1], quatPlace[2], quatPlace[3]);
	  ...
	  if (SorF==1)
	  {
	    regrasp.StandtoLie(0,-30,105,-0.5);
	  ...
	  */
      ...
      hand.WaitRest(0.250);
      ROS_INFO("Hand Opened");
      ROS_INFO("Regrasp Vibrate Completed!");
    }
