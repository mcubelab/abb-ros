...
int main(int argc, char** argv)
{
  ...
  ROS_INFO("Prehensile Pushing pivoting demo...");
 ...
 // HomogTransf pose;
  ...
  Quaternion quatR = Quaternion("0.0 0.0 -1.0 0.0"); // Hand rotated fpor linpush
  Quaternion quatR2 = Quaternion("0.0 0.7071 0.0 0.7071"); //hand rotated for pivoting
  Vec transPick= Vec("1300 300 52",3); //for hex object 1300 300 58; // for rect 1300 300 77
  Vec transLift= Vec("1300 300 300",3);  // Lift the hand upto this point while the object is drooping in the hand.
  Vec transPlace= Vec("1300 300 150",3);  // Place Point after droop action
  Vec transPush_safe= Vec("1500 260 147",3);  // before and after push z=240 is center (center ~240 for hex and for rect = ~235)
  Vec transPush_end= Vec("1588 260 147",3); //1585 object touches ; z= 127 is centerline
  ...
  robot.SetJoints(0,0,0,0,90,-50);
  
  robot.SetJoints(0,0,0,80,90,-50);

  robot.SetCartesian(transLift[0], transLift[1], transLift[2], quatR2[0], quatR2[1], quatR2[2], quatR2[3]);
  
  robot.SetCartesian(transPush_safe[0], transPush_safe[1], transPush_safe[2], quatR2[0], quatR2[1], quatR2[2], quatR2[3]);
  ...
  robot.SetCartesian(transPush_end[0], transPush_end[1], transPush_end[2], quatR2[0], quatR2[1], quatR2[2], quatR2[3]);
